...
function buildScalingPanel(app, actor, scaling) {
  ...
  hint.className = "party-scaling-hint";
  hint.textContent = getScalingSummary(scaling);
  ...
  detail.className = "party-scaling-detail";
  detail.textContent = getModifierDetailLabel(scaling);

  const source = document.createElement("p");
  source.className = "party-scaling-source";
  source.textContent = getScalingSourceLabel(scaling);
  ...
  body.append(checkboxRow, hint, modifier, detail, source);
  panel.append(header, body);
  return panel;
}

function getScalingSummary(scaling) {
  if (scaling.partyLevel === null) {
    return game.i18n.localize(`${MODULE_ID}.sheet.noParty`);
  }

  if (!scaling.enabled) {
    return game.i18n.format(`${MODULE_ID}.sheet.disabled`, {
      npcLevel: scaling.baseLevel,
      partyLevel: scaling.partyLevel,
      delta: formatSignedNumber(scaling.delta),
    });
  }

  if (scaling.delta === 0) {
    return game.i18n.format(`${MODULE_ID}.sheet.match`, {
      partyLevel: scaling.partyLevel,
    });
  }

  return game.i18n.format(`${MODULE_ID}.sheet.enabled`, {
    npcLevel: scaling.baseLevel,
    partyLevel: scaling.partyLevel,
    delta: formatSignedNumber(scaling.delta),
  });
...
function getScalingSourceLabel(scaling) {
  if ((scaling.source === "active-party-level" || scaling.source === "active-party-members") && scaling.partyName) {
    return game.i18n.format(`${MODULE_ID}.sheet.sourceActiveParty`, {
      name: scaling.partyName,
    });
  }

  if (scaling.source === "characters") {
    return game.i18n.localize(`${MODULE_ID}.sheet.sourceCharacters`);
  }

  return game.i18n.localize(`${MODULE_ID}.sheet.sourceNone`);
}
...
function getModifierDetailLabel(scaling) {
  if (scaling.partyLevel === null) {
    return game.i18n.localize(`${MODULE_ID}.sheet.detailUnavailable`);
  }

  if (scaling.rawDelta === scaling.delta) {
    return game.i18n.format(`${MODULE_ID}.sheet.rawModifier`, {
      delta: formatSignedNumber(scaling.rawDelta),
    });
  }
...
function getPartyLevelData() {
  ...
  if (partySource === "active-party-level-first" || partySource === "active-party-level-only") {
    const activePartyLevel = Number(activeParty?.level);
...
function getBaseNpcLevel(actor) {
  const level = Number(actor?.system?.details?.level?.base ?? actor?._source?.system?.details?.level?.value ?? actor?.level ?? 0);
